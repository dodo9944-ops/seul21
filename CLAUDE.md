...
| 대상 | 경로 | 비고 |
|------|------|------|
| 자료실 데이터 | `redevelopment/assets/data/mock-data.js` → `MOCK.library` | 뉴스·법령·판례·서식·사례 전체 |
...
### 3. 정책 오독 방지 — 기한 기반 자동 삭제 금지

**자료실 보관 기한**: **1년 (365일)** — `보도자료 게시·백업 정책(2026-04-15)` 참조.

금지 사례 (2026-04-19 사고 재발 방지):
- ❌ "7일 초과"를 자료실 삭제 기준으로 오해하여 삭제 → 7일은 **홈페이지 노출 기준**일 뿐
- ❌ "1년 초과"라도 **자동 삭제 금지** → `archive/news_YYYY.js`로 **백업 후 제거** (분기 1회, 사용자 확인 필수)
